...
export const UtilsProvider = ({ children }) => {
  ...
  const moveToWishlistHandler = async (item) => {
    try {
      ...
      if (isItemInWishlist(item)) {
        // If it is, just fetch the wishlist
        await fetchWishlist();
      } else {
        // If it's not, add the item to the wishlist
        await postToWishlist(item);
      }
  
      // Update wishlist and cart after ensuring deletion from the cart and addition to the wishlist
      updateWishlist();
      updateCart();
    } catch (err) {
      console.error(err);
